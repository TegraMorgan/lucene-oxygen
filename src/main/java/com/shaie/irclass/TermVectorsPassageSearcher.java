...
public class TermVectorsPassageSearcher implements com.shaie.irclass.PassageSearcher {
     ...
     * @param overlapRatio  the percentage of overlapping characters between passages.
        ...
        for (final ScoreDoc sd : topDocs.scoreDocs) {
            ...
            if (terms == null) {
                throw new IllegalStateException(
                        format("Document [%d] does not have term vectors indexed for field [%s]", sd.doc, field));
            }
                ...
                for (int i = 0; i < postings.freq(); i++) {
                    ...
                    for (final Passage docPassage : docPassages) {
                        if (docPassage.getStartOffset() <= startOffset && endOffset <= docPassage.getEndOffset()) {
                            ...
                            docPassage.addTerm(qTerm.text(), new Interval(startOffset, endOffset));
